...
class EDDeconv(nn.Module):
    def __init__(self, cin, cout, zdim=128, nf=64, activation=nn.Tanh):
        ...
        network = [
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(nf*32, zdim, kernel_size=4, stride=1, padding=0, bias=False),  # 4x4 -> 1x1
            nn.ReLU(inplace=True)]
        ## upsampling
        network += [
            nn.ConvTranspose2d(zdim, nf*32, kernel_size=4, stride=1, padding=0, bias=False),  # 1x1 -> 4x4
            nn.ReLU(inplace=True),
            nn.Conv2d(nf*32, nf*32, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(nf*32, nf*16, kernel_size=4, stride=2, padding=1, bias=False),  # 4 -> 8
            nn.GroupNorm(16*16, nf*16),
            nn.ReLU(inplace=True),
            nn.Conv2d(nf*16, nf*16, kernel_size=3, stride=1, padding=1, bias=False),
            nn.GroupNorm(16*16, nf*16),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(nf * 16, nf * 8, kernel_size=4, stride=2, padding=1, bias=False),  # 8 -> 16
            nn.GroupNorm(16 * 8, nf * 8),
            nn.ReLU(inplace=True),
            nn.Conv2d(nf * 8, nf * 8, kernel_size=3, stride=1, padding=1, bias=False),
            nn.GroupNorm(16 * 8, nf * 8),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(nf * 8, nf * 4, kernel_size=4, stride=2, padding=1, bias=False),  # 16 -> 32
            nn.GroupNorm(16 * 4, nf * 4),
            nn.ReLU(inplace=True),
            nn.Conv2d(nf * 4, nf * 4, kernel_size=3, stride=1, padding=1, bias=False),
            nn.GroupNorm(16 * 4, nf * 4),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(nf * 4, nf * 2, kernel_size=4, stride=2, padding=1, bias=False),  # 32 -> 64
            nn.GroupNorm(16 * 2, nf * 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(nf * 2, nf * 2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.GroupNorm(16 * 2, nf * 2),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(nf*2, nf, kernel_size=4, stride=2, padding=1, bias=False),  # 64 -> 128
            nn.GroupNorm(16, nf),
            nn.ReLU(inplace=True),
...
class ConfNet(nn.Module):
    def __init__(self, cin, cout, zdim=128, nf=64):
        ...
        ## upsampling
        network += [
            nn.ConvTranspose2d(zdim, nf*32, kernel_size=4, padding=0, bias=False),  # 1x1 -> 4x4
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(nf*32, nf*16, kernel_size=4, stride=2, padding=1, bias=False),  # 4x4 -> 8x8
            nn.GroupNorm(16*16, nf*16),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(nf*16, nf*8, kernel_size=4, stride=2, padding=1, bias=False),  # 8x8 -> 16x16
            nn.GroupNorm(16*8, nf*8),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(nf * 8, nf * 4, kernel_size=4, stride=2, padding=1, bias=False),  # 16 - 32
            nn.GroupNorm(16 * 4, nf * 4),
        ...
        out_net1 = [
            ...
            nn.GroupNorm(16, nf),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(nf, nf, kernel_size=4, stride=2, padding=1, bias=False),  # 128-256
            nn.GroupNorm(16, nf),
            nn.ReLU(inplace=True),
            nn.Conv2d(nf, 2, kernel_size=5, stride=1, padding=2, bias=False),  # 256
            nn.Softplus()]
        self.out_net1 = nn.Sequential(*out_net1)

        out_net2 = [
            nn.Conv2d(nf*2, 2, kernel_size=3, stride=1, padding=1, bias=False),  # 64
            nn.Softplus()]
